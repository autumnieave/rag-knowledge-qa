...
本原型以「公考申论智能问答」为示例业务场景：公考培训与教育领域大量存在"基于给定材料作答"的知识问答需求（题库练习、范文讲解、知识点问答），核心难点是**答案必须严格基于给定材料、不能编造、且可溯源**。本项目用 RAG 链路验证了这条核心路径：

- 问题 → 本地嵌入模型编码 → 向量库检索给定材料 → LLM 基于材料作答 → 返回引用来源
- 30 题封闭集回归：材料检索全部命中、要点覆盖 100%、预热后单题约 2.6s（详见 [评估报告.md](评估报告.md)）

> **项目来源**：本原型源于某次 AI 编码实习笔试（指定以公考申论为场景构建知识库问答接口），代码、评估与文档均为本人实现。
